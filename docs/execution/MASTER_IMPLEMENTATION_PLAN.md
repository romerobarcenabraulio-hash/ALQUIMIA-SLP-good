...
| B · Plataforma 0/backoffice | Habilitar administracion interna de tenants, gates, capabilities y evidencia. | `frontend/src`, `backend`, `docs/architecture/PLATAFORMA_0*`, rutas admin. | Bloque A; ADR-0010; spec Plataforma 0. | Founder puede administrar tenant, gates, capabilities y evidencia sin exponer backoffice al cliente. | Backoffice decorativo, permisos ambiguos. | Landing, marketing, aprobaciones automaticas. | KRONOS + POLIS + AUDITOR. |
...
2. Ejecutar B-C-D antes de cualquier tenant nuevo.
