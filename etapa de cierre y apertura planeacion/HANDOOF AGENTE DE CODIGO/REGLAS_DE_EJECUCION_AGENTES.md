...
- `git pull origin main` antes de empezar. Rama de vida corta por tarea. **NUNCA commit directo a `main`.**
- Un PR por tarea, descripción clara, **tests verdes con la salida real pegada** (anti-mentira). Merge verificado el mismo día.
...
Ninguna tarea se palomea (pasa a Done) hasta completar este ciclo. **Codificas → te auto-auditas → corriges → palomeas → sigues.**
...
4. ABRIR PR → review de Greptile + CI verde (gate independiente).
5. Solo entonces PALOMEAR (Done) y tomar la SIGUIENTE tarea desbloqueada del tablero.
```
...
- **PALOMEAR (Done):** solo tras merge gated; lo hace quien tiene el gate, no el agente.
