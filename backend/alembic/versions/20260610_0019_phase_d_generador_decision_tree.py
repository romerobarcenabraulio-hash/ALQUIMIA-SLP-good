...
def upgrade() -> None:
    ...
    if not _has_table(bind, 'generator_residue_records'):
        op.create_table(
    ...
    if not _has_table(bind, 'municipal_residue_aggregates'):
        op.create_table(
    ...
    if not _has_table(bind, 'decision_tree_sessions'):
        op.create_table(
    ...
    if not _has_table(bind, 'compliance_templates'):
        op.create_table(
...
def downgrade() -> None:
    bind = op.get_bind()
    if _has_table(bind, 'compliance_templates'):
        op.drop_table('compliance_templates')
    if _has_table(bind, 'decision_tree_sessions'):
        op.drop_table('decision_tree_sessions')
    if _has_table(bind, 'municipal_residue_aggregates'):
        op.drop_table('municipal_residue_aggregates')
    if _has_table(bind, 'generator_residue_records'):
        op.drop_table('generator_residue_records')
    if _has_table(bind, 'generador_entities'):
